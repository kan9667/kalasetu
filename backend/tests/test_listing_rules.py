...
init_db()
_db = SessionLocal()
if not _db.query(ArtisanDB).filter(ArtisanDB.id == "artisan_rules_test").first():
    _db.add(ArtisanDB(id="artisan_rules_test", name="Rules Artisan", phone="+919876543212"))
    _db.commit()
_db.close()

from backend.tests.conftest import IdempotentTestClient
token = create_access_token("artisan_rules_test")
